...
def rate(ticker, mode):
    try:
        ...
        pricevor1 = Euro(
            get_data(ticker, mode="hprice", sdate=incomevor1day, fdate=incomevor1day)[0]["historical"][0]["close"],
            quote_currency)
        ...
        Gesamtscore = round(
            KGVScore + MargeScore + EKQScore + DividendyieldScore + UmsatzScore + LiquidityScore + DCFScore + GewinnwachstumScore + KWGWVScore + PayoutRatioScore,
            2)
...
def compare(tickerliste):
    ...
    for index, ticker in enumerate(tickerliste):
        ...
        flist.append(rating)
        print("Ticker {0}/{1} gerated. ({2})".format(index + 1, len(tickerliste), ticker) + " " * (
                    len(tickerliste[tickerliste.index(ticker) - 1]) - len(ticker)), end="\r")
    ...
    for index, rating in enumerate(failed):
        ...
        if index == 0:
            returnstring += "\n"
        returnstring += "{0}. {1}{2}{3}\n".format(index + 1 + len(highest) + len(flist), rating[1], "\t" * tabs,
                                                  rating[0][0])
...
def rateUmsatz(umsatz):
    schwellenwerte = [500000000, 5000000000, 15000000000, 50000000000, 120000000000, 200000000000, 250000000000,
                      250000000000]
...
def askforpref(k_index, total):
    k_strings = ["der KGV", "die Ebitda-Marge", "die Eigenkapitalquote", "die Dividendenrendite", "der Umsatz",
                 "die Aktienliquidität", "das Kurs-zu-DCF-Verhältnis",
                 "das Verhältnis von Kurswachstum zu Gewinnwachstum", "das Payout-Ratio", "das Gewinnwachstum"]
